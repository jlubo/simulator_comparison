...
### Copyright 2017-2024 Jannik Luboeinski
...
# Computes and prints the goodness of fit between two datasets (quantified by CV, R^2, and RMSE, using methods
# from scikit-learn). For the size of the dataset, considers a tolerance.
# data_1: the first dataset
# data_2: the second dataset
# tol [optional]: tolerance for the lengths of the datasets (within the tolerance, the larger dataset is pruned)
# print_results [optional] : indicates if the computed results shall be printed
# return: tuple of the results
def computeGoodnessOfFit(data_1, data_2, tol=500, print_results=True):
	# if a dataset is too large by maximally the tolerance, save it and prune it
	diff_len = len(data_1) - len(data_2)
	if diff_len == 0:
		saved_data_1 = None
		saved_data_2 = None
	elif abs(diff_len) < tol:
		if diff_len < 0:
			saved_data_1 = None
			saved_data_2 = copy.deepcopy(data_2)
			data_2 = data_2[:-abs(diff_len)]
		elif diff_len > 0:
			saved_data_1 = copy.deepcopy(data_1)
			saved_data_2 = None
			data_1 = data_1[:-abs(diff_len)]
	else:
		raise ValueError("Array sizes do not match (within tolerance): "
		                 f"{len(data_1)} for 'data_1', {len(data_2)} for 'data_2'.")

	# compute CV using scikit-learn
	...
	# compute R^2 using scikit-learn
	rsq = sklearn.metrics.r2_score(data_1, data_2)

	# print the R^2 result
	if print_results:
		print(f"  R^2 = {rsq}")

	# compute RMSE using scikit-learn
	...
	#np.savetxt("pruned_data.txt",np.column_stack((data_1, data_2)))

	# copy back the original data
	if saved_data_1 is not None:
		data_1 = copy.deepcopy(saved_data_1)
	if saved_data_2 is not None:
		data_2 = copy.deepcopy(saved_data_2)
