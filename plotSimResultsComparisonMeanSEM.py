...
from utilityFunctions import hasTimestamp, computeGoodnessOfFit

# TODO:
# * consistency checks as in old 'plotResultsSimulatorComparison.py' (see `count*Data` in 'utilityFunctions.py')
# * plotting the total weight, as for example in 'STET_comp_plot.gpl'
# * put main plotting function in separate module
# * selection of different sampling methods
# * more generality (in contrast to current specific-model context)
# * possibly, class implementation with `X_cols` etc. as attributes

#####################################
# plotMeanAndSEMofX
# Plots the mean and the s.e.m. across `n_batches` for given values of synaptic weight, membrane voltage, and calcium/
# protein concentration. Deterministic data can be plotted as well, corresponding to `n_batches = 1`, with no s.e.m. 
# considered.
...
# - data_stacked_1: two-dimensional array containing the values of the membrane potential, weights, calcium and protein
#                   concentration, etc. over time
...
# - data_stacked_2 [optional]: two-dimensional array containing the values of the membrane potential, weights, calcium
#                              and protein concentration, etc. over time (may be left out)
# - spikes_stacked_2 [optional]: two-dimensional array containing the neuron and time of each spike (may be left out)
# - X_cols [optional]: dictionary specifying the data columns of the measures whose mean and s.e.m. is to be plotted
#                      (must point to the mean, the std.dev. is sought in the next column; if a value is <= 0, the
#                      related variable is not plotted)
...
def plotMeanAndSEMofX(X_name, n_batches, config, 
	...
	if X_cols.get("weight", 0) > 0:
		...
		if data_stacked_2 is not None:
			...
			print(f"Goodness of fit for {X_name} of weight ({framework_name_1} - {framework_name_2}):")
			try:
				computeGoodnessOfFit(data_stacked_1[:,X_cols["weight"]], data_stacked_2[:,X_cols_2["weight"]], print_results=True)
			...
			if X_cols_2.get("weight_l", 0) > 0:
				...
				print(f"Goodness of fit for {X_name} of late weight ({framework_name_1} - {framework_name_2}):")
				try:
					computeGoodnessOfFit(data_stacked_1[:,X_cols["weight_l"]], data_stacked_2[:,X_cols_2["weight_l"]], print_results=True)
				except ValueError as err:
					print(err)
	...
	if X_cols.get("voltage", 0) > 0:
		...
		if data_stacked_2 is not None:
			...
			print(f"Goodness of fit for {X_name} of membrane voltage ({framework_name_1} - {framework_name_2}):")
			try:
				computeGoodnessOfFit(data_stacked_1[:,X_cols["voltage"]], data_stacked_2[:,X_cols_2["voltage"]], print_results=True)
			except ValueError as err:
					print(err)
	...
	# Calcium dynamics
		if X_cols.get("calcium", 0) > 0:
			...
			if data_stacked_2 is not None:
				...
				print(f"Goodness of fit for {X_name} of calcium concentration ({framework_name_1} - {framework_name_2}):")
				try:
					computeGoodnessOfFit(data_stacked_1[:,X_cols["calcium"]], data_stacked_2[:,X_cols_2["calcium"]], print_results=True)
				except ValueError as err:
					print(err)
	...
	# Protein dynamics
		if X_cols.get("protein", 0) > 0:
			...
			if data_stacked_2 is not None:
				...
				print(f"Goodness of fit for {X_name} of protein concentration ({framework_name_1} - {framework_name_2}):")
				try:
					computeGoodnessOfFit(data_stacked_1[:,X_cols["protein"]], data_stacked_2[:,X_cols_2["protein"]], print_results=True)
				except ValueError as err:
					print(err)
